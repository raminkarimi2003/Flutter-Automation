...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
// The purpose of this test case is to assert that the Rental period remaining in the drive is the Rental period
//  set in Calendar.
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
public class AssertRentalPeriodInDrive extends BaseIOSTest{
 
	@Test(groups={"TestGroup"})
	public void AssertRentalPeriodInDrive_Reset(ITestContext context) throws InterruptedException, IOException
	{		
		...
		Thread.sleep(5000);
		SelectAssetForRent.ScrollDownTo(driver,"BACKHOE"); // scroll down to asset ("BACKHOE") and select it
		int RentalPeriod=22;  // represent the day(s) including today
		CalendarDate.setRentalDate(driver,RentalPeriod);		
		new ScrollToText(driver);
		ScrollToText.scroll("Start Rental");   // scroll to rental button.
        StartRental.rent(driver,"Start Rental");  // rent   
        resources.Listeners.setTestCaseBooking(true);
        equipmentDetailsPage eqp=new equipmentDetailsPage(driver);
        String stringID=eqp.equipmentDetailsID.getText();
        System.out.println("ID is "+stringID);
        String bookingID=stringID.substring(10);
		...
		int RemainingPeriod=RentalPeriod-1;
	    Assert.assertEquals("Rental ends in "+RemainingPeriod+" day(s)",drive.rentalRemaining.getText());
