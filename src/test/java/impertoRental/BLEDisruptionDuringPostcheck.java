...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// This test case will assert that the app can gracefully handle the BLE disruption during post-check and can
// complete its renting process, after BLE restoration.
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
public class BLEDisruptionDuringPostcheck extends BaseIOSTest{
 
	@Test(groups={"TestGroup1"})
	public void BLEInteruptPostcheck_NoReset() throws InterruptedException, IOException
	{		
		...
		driver.launchApp();
		Thread.sleep(2000);
		// This conditional flag is added because of inconsistency behavior in the app to BLE Interruption
		boolean conditionalFlag=eq.searchEquipment.size()>0; // if equipment details page present
		if(conditionalFlag)
			eq.RentedEquipment.click();	
		else 
		{	
			IsEquipmentDetailsPagePresent.Check(driver,"Booking ID");    // check to see if this page present
			eq.RentedEquipment().click();
		}
		...
		if(conditionalFlag)
		{
			System.out.println("End button not displayed");
			eq.Equipment.click();
			Thread.sleep(4000);
			UseEquipment.use(driver, "Use Equipment"); 
		}
