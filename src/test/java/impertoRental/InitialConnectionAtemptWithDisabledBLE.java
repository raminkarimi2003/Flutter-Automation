...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// This test case will attempt to connect to asset with the disabled BLE, and when the connection is refused with
// the appropriate message, then it will switch BLE back on again and will complete the renting process by submitting
// all the pre and post checklist.
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
public class InitialConnectionAtemptWithDisabledBLE extends BaseIOSTest{
 
	@Test(groups={"TestGroup4"})
	public void BLEInteruptInitialConnection_NoReset() throws InterruptedException, IOException
	{		
		service=startServer();  // start server
		BaseIOSTest.Credential();  // invoke method to access username and/or password properties
		//BaseIOSTest_NoReset.Credential();  // invoke method to access username and/or password properties
        ...
        StartRental.rent(driver,"Start Rental");  // rent
       	// Navigate to iPhoned setting to toggle the Bluetooth
	    ...
	    UseEquipment.use(driver, "Use Equipment");//	use Equipment	
	    Thread.sleep(3000);  // wait
	    connectToEquipmentPage conn=new connectToEquipmentPage(driver);
	    Assert.assertTrue(true, conn.notConnected.getText());  // assert that Equipment cannot be connected without Blutooth enabled
	    driver.activateApp("com.apple.Preferences");   // activate iPhone setting
        new EnableBluetooth(driver);
		EnableBluetooth.enable(true);   // switch blue tooth ON
		driver.closeApp();
		driver.launchApp();  // launch the main app and continue with the booking
		ScrollToText.scroll("Use Equipment");   // scroll to "Use Equipment" button
	    UseEquipment.use(driver, "Use Equipment");//	use Equipment	
